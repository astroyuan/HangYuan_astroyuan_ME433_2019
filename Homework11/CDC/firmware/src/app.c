...
#define MAF_BUFFER_SIZE 6
#define FIR_BUFFER_SIZE 6
int len, i = 0;
...
int IMU_flag = 1;
    ...
    int MAF_size = MAF_BUFFER_SIZE;
    int FIR_size = FIR_BUFFER_SIZE;
    short MAF_buffer[MAF_BUFFER_SIZE];
    float MAF_cof = 1.0/MAF_BUFFER_SIZE;
    
    short FIR_buffer[FIR_BUFFER_SIZE];
    float FIR_cof[FIR_BUFFER_SIZE];
    
    short IIR_prev = 0, IIR_new = 0;
    float IIR_cof1 = 0.3;
    float IIR_cof2 = 0.7;
    
    short accelZ_MAF, accelZ_FIR, accelZ_IIR;
    
    int MAF_buffer_start = 0, MAF_buffer_end = 0;
    int FIR_buffer_start = 0, FIR_buffer_end = 0;
    int cnt = 0;
    float accelZ_FIR_tmp;
    float accelZ_MAF_tmp;
// *****************************************************************************
/* Application Data
  Summary:
...
void APP_Initialize(void) {
    ...
    memset(MAF_buffer, 0 , sizeof(MAF_buffer));
    memset(FIR_buffer, 0 , sizeof(FIR_buffer));
    memset(FIR_cof, 0 , sizeof(FIR_cof));
    FIR_cof[0] = 0.0264;
    FIR_cof[1] = 0.1405;
    FIR_cof[2] = 0.3331;
    FIR_cof[3] = 0.3331;
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            // MAF
            MAF_buffer[MAF_buffer_end] = accelZ;
            MAF_buffer_start = (MAF_buffer_end + 1) % MAF_size;
            accelZ_MAF_tmp = accelZ;
            while (MAF_buffer_start != MAF_buffer_end)
            {
                accelZ_MAF_tmp += MAF_buffer[MAF_buffer_start];
                MAF_buffer_start = (MAF_buffer_start + 1) % MAF_size;
            }
            MAF_buffer_end = (MAF_buffer_end + 1) % MAF_size;
            accelZ_MAF = (short) (accelZ_MAF_tmp * MAF_cof);
            
            // IIR
            IIR_new = accelZ;
            accelZ_IIR = (short) (IIR_cof1 * (float)IIR_prev + IIR_cof2 * (float)IIR_new);
            IIR_prev = IIR_new;
            
            // FIR
            FIR_buffer[FIR_buffer_end] = accelZ;
            FIR_buffer_start = (FIR_buffer_end + 1) % FIR_size;
            accelZ_FIR_tmp = accelZ * FIR_cof[FIR_size-1];
            cnt = 0;
            while (FIR_buffer_start != FIR_buffer_end)
            {
                accelZ_FIR_tmp += FIR_cof[cnt] * FIR_buffer[FIR_buffer_start];
                FIR_buffer_start = (FIR_buffer_start + 1) % FIR_size;
                cnt++;
            }
            FIR_buffer_end = (FIR_buffer_end + 1) % FIR_size;
            accelZ_FIR = (short) accelZ_FIR_tmp;
            
            //len = sprintf(dataOut, "%d\r\n", i);
            //i++; // increment the index so we see a change in the text
            len = sprintf(dataOut, "%d %i %i %i %i\r\n", i+1, accelZ, accelZ_MAF, accelZ_IIR, accelZ_FIR);
            i++;
